...
loginRouter.post("/api/login", async (req, res) => {
    ...
    const correctPassword: string = registeredUser ? await bcrypt.compare(body.password, registeredUser.passwordHash) : false;
    ...
    res.status(200).json({ token, username: registeredUser.username, isAdmin });
});
